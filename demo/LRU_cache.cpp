...
class LRUCache : public Cache, public CacheNode{
private:
...
// 캐시 클래스 두 함수를 사용하기 위해 실질적으로 사용하는 클래스이다. (LFU)
...
public:
    // 생성시, key 0, value 0으로 전부 초기화...  그렇지만,,, 0으로 lookup해보면 없음...
    LFUCache(int num):MaxNumEntries(num){   
        //head <-> tail
        head = new Item("0", 0, 0);
        tail = new Item("0", 0, 0);
        head->next = tail;
    ...
    bool lookup(string k, int &val, int &byte){
        ...
        val = (*it).second->value; //found, 발견 했을 경우, 그 노드가 가지고 있는, val값을 리턴해준다.
        byte = (*it).second -> byte; // 바이트를 추가한다.
        /* update linked list LFU는 굳이 업데이트를 해줄 필요가 없다?
        detach((*it).second); //detach, 그 자리에서 지우고, 새로 맨 앞으로 가져온다.
        push_front((*it).second); //add to the head because this is the latest, 가장 최신으로 업데이트 한다.
        */
        return true;
    ...
    void put(string k, int val, int byte){
        ...
        if(CacheEntries.size() != MaxNumEntries){ // 집어넣으려고 할떄, 캐시가 꽉 차지 않은 경우,
            ...
            /* Select Victim & delete */
            // 캐시가 꽉 찼다, 그러므로, 하나를 삭제해야 하는데, 가장 최신 것을 삭제해야 한다.(LFU)
            string key;
            /*------삭제 관련 로직(LFU)--------*/
            back(key); //get the key of the newest one
            pop_back();       //remove the oldest one
            auto rt = CacheEntries.find(key);
            if(rt != CacheEntries.end()){ //found
                CacheEntries.erase(rt); //delete
...
class Requester
{
    ...
    public:
        ...
        Item send_packet()
        {
            random_device rd;
            mt19937_64 rng(rd());
            uniform_int_distribution<int64_t> cnt(0, 200);
...
class Provider : public Requester
{
    private:
        Item *r_packet[200];
        int random;
...
/*
    ...
    이떄 캐시 정책은 LRU(least recently used) 무튼 가장 오래 된 것을 삭제하고, 새로운 패킷이 끝에서 부터 들어오게 된다.
...
int main(){
    ...
    } else if (c_cache == 2)
    {
        ...
        while (x < 200){
            ...
            //cache.put(it.key,it. value, it.byte);
            if (cache2.lookup(it.key,it. value, it.byte)){
                cout << k << "캐시에 존재!!. 반환해줄게...\n";
                c_hit++;
            } else {
                cout << "프로듀서에게 전달하도록 할게 ^^" << endl;
                c_miss++;
                auto dt = Data.receive_packet(it.key,it. value, it.byte);
                cache2.put(dt.key, dt.value, dt.byte);
            }
            x++;
        }
        cout << "현재 큐에 남아 있는 데이터:\n";
